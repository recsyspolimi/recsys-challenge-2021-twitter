...
dict_name = "language_id"
out_cols = ["mapped_language_id"]
...
if __name__ == '__main__':
    ...
    # Load dataset
    df = read_dataset(dataset_path, ["raw_feature_tweet_language"])

    # Create Dict
    if generate_dict:
        create_and_save_uniques(dict_path,
                                dict_name,
                                df["raw_feature_tweet_language"])

    # Map the columns
    out = load_uniques_create_dict_map(c, dict_path, dict_name,
                                       df["raw_feature_tweet_language"], out_cols[0], np.uint8).to_frame()
