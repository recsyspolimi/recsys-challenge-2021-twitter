...
class RecommenderGBM(RecommenderBase, ABC):
    '''Defines the structure that all the Models expose'''

    def __init__(self, batch: bool = False, name: str = "recommenderbase", kind: str = "NOT_GIVEN"):
    ...
    # TODO: Redundant AF
    # ------------------------------------------------------
    def save_model(self, filename: str = None, path: str = None) -> str:
        # Defining the extension
        # Saving the model with premade name in working folder
        if (path is None) and (filename is None):
            date = str(dt.datetime.now().strftime("%d_%m_%Y_%H_%M_%S"))
